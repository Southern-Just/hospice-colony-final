...
type Props = {
  ...
  onClose: () => void;
  onUpdate?: (hospital: HospitalRecord) => void;
};

export function HospitalModal({ hospital, onClose, onUpdate }: Props) {
  const { user: currentUser } = useAuth();
  const [form, setForm] = useState<HospitalRecord | null>(hospital || null);
  ...
  const handleWardsChange = useCallback((wards: WardRecord[]) => {
    setForm(prev => {
      if (!prev) return prev;
      return { ...prev, wards };
    });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl p-6 relative overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <div className="text-xs text-gray-500">Viewing hospital ID: {hospital.id.substring(0, 8)}...</div>
