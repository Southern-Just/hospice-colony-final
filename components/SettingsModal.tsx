...
export function SettingsModal({ onClose }: Props) {
  ...
  const [userData, setUserData] = useState<Omit<User, 'role' | 'hospitalId' | 'hospitalName'> | null>(null)
  ...
  const fetchUser = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!data.user) throw new Error('No user object returned')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!data.user) throw new Error('No user object returned after update')
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                value={initialData.hospitalName ?? 'Not affiliated with a hospital'}
                disabled
                className="bg-gray-100 cursor-not-allowed"
              />
            </div>
